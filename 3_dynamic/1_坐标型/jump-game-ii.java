// version 1: Dynamic Programming, 复杂度是 O(n^2)，会超时，但是依然需要掌握。
public class Solution {
    public int jump(int[] A) {
        //state
        int[] f = new int[A.length]; //state: f[i]表示从位置0到位置i最少需要多少步
        //init
        f[0] = 0;
...
i=5 f[5] = 3
i=6 f[6] = 3
i=7 f[7] = 3 //为什么f[i] = f[j] + 1; break;  因为f[i] = min(f[i], f[j] + 1), 刨去+1不看, 实际就是取一个f[j]最小的, f[j]一定是非递减的顺序, 所以找到第一个满足条件的f[j]即可
...
/*
 0   1 2   3 4   5 6 7 8
[2] [3 1] [5 2] [1 3 1 -]
上面这个例子, 第一次搜索2, 然后更新搜索范围到了坐标1-2. 第二次搜索1-2坐标对应的跳数3,1, 最远可以到4, 更新搜索范围到了坐标3-4, 第三次搜索3-4坐标对应的5,2, 更新搜索范围到了坐标5-8, 然后发现其超过了数组当前额长度, 说明可以跳到, 且是最短跳数
