public class Solution {
    public boolean isMatch(String s, String p) {
        if(s.length() == 0){
            ...
            return true; //当s的长度是0, 只有p全是*时, 或者p也是0才能true
        }
        if (s == null){
            return p == null;
        }
        ...
        // state
        boolean[][] f = new boolean[s.length() + 1][p.length() + 1]; //f[i][j]表示S的前i个字符能否和T的前j个字符匹配。
        // init
        f[0][0] = true;
        for(int j = 1; j <= p.length(); j++){ //即只有*才可能和空字符匹配
            if(f[0][j - 1] && p.charAt(j - 1) == '*'){
                f[0][j] = true;
            }
        }
        // function
...
情况2. T(j) == *，此时分为两种情况。
    ...
    第二种情况不让*和S(i)匹配(也可以看成*匹配为空字符），此时看result[i][j - 1]的值。
