...
public class Solution {
    public boolean firstWillWin(int[] values) {
        ...
        return allsum < 2 * MemorySearch(0, n - 1, f, flag, values, sum); //表示从第0到第n-1的硬币中取, 先手能获得的最大价值
    }
    int MemorySearch(int left, int right, int[][] f, boolean[][] flag, int[] values, int[][] sum) {
        if(flag[left][right]) {  
            return f[left][right];
        }
        if(left > right) {
