public class Solution { //直接前向dp
    public boolean firstWillWin(int n){
        ...
        // state
        boolean[] f = new boolean[n + 1]; //f[i]表示剩下i个硬币时, 当前选手能否赢
        f[0] = false;
        f[1] = true;
        f[2] = true;
        // function
...
public class Solution { // lintcode上这个会栈溢出
    public boolean firstWillWin(int n) {
        ...
        return MemorySearch(n, f, flag);
    }
    boolean MemorySearch(int i, boolean[] f, boolean[] flag) {
        if(flag[i] == true) {
            return f[i];
        }
        if(i == 0) { //一个都不剩, 说明拿不到最后一个, 所以是false
            ...
            f[i] = !MemorySearch(i - 1, f, flag) || !MemorySearch(i - 2, f, flag);
        }
        flag[i] =true;
        return f[i];
    }
