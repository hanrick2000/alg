public class Solution {
    public int numDistinct(String S, String T) {
        ...
        //state
        int[][] f = new int[S.length() + 1][T.length() + 1]; //f[i][j]表示S的前i个字符中选取T的前j个字符, 有多少中挑选方案
        //init
        for (int i = 0; i <= S.length(); i++) {
            f[i][0] = 1; //从字符串中挑出空串, 总会是有1个方案
        }
        //function
        for (int i = 1; i <= S.length(); i++) {
            for (int j = 1; j <= T.length(); j++) {
                if (S.charAt(i - 1) == T.charAt(j - 1)) {
                    f[i][j] = f[i - 1][j] + f[i - 1][j - 1]; //最后一个字符相等: 不配对最后一个字母 + 配对最后一个字母
                }else{
                    f[i][j] = f[i - 1][j]; //最后一个字符不等: 转化成S的前i-1个字符中选取T的前j个字符的挑选方案数
                }
            }
        }
        //result
