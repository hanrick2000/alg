public class Solution {
    public int stoneGame(int[] A) {
        ...
        int n = A.length;
        // state
        int[][] f = new int[n][n]; //f[i][j]表示把第i个到第j个石子合并在一起的最小花费
        // initialize
        for (int i = 0; i < n; i++) {
            f[i][i] = 0; //f[0][0], f[1][1]...都是0, 因为不需要合并
        }
        // preparation
        int[][] sum = new int[n][n]; //sum[l][r]表示从l到r的石子和
        for (int i = 0; i < n; i++) {
            sum[i][i] = A[i];
            for (int j = i + 1; j < n; j++) {
...
3. Merge the last two piles =>     [10], score +10 // 合并完所有的前一步是[6,4], 6是通过4和2合并来的, 另一个4不用合并; 2是通过1和1合并来的, 得到2的消耗是2, 得到6的消耗是
Other two examples:
[1, 1, 1, 1] return 8
[4, 4, 5, 9] return 43
合并石子, 得到最小价值
