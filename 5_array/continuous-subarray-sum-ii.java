public class Solution {
    public ArrayList<Integer> continuousSubarraySumII(int[] A) {
        ...
        local = 0;
        //找最小子数组，数组总和减去最小子数组即为有循环情况下最大子数组
        for(int i = 0; i < A.length; i++){
            if(local <= 0){
                local += A[i];
                end = i;
...
/*
这道题和continues subarray sum I思路一样，不同之处在于可以从成环，
因此解法稍有不同。总共分为两种情况讨论:
1 首先遍历数组找出没有环的情况下最大的子数组之和, 方法和I相同.
2 然后找有环情况下最大子数组之和. 做法为: 
  寻找数组中最小和的连续子数组,
  然后用整个数组之和减去这个最小和的连续子数组剩下的就是该情况下的最大和的子数组.
  将该情况下得到的最大和与1中得到的最大和相比, 取和更大的那种即可.
3 几个tips:
  1) 在1中遍历时, 可以顺便求出整个数组的和.
  2) 2中要考虑一种情况为整个数组全部为负数, 这样需要减去的是整个数组, 因此返回1中的结果.
  3) 根据2中求的最小数组, 最大数组的index因该为end＋1到start－1, 
...
find a continuous subarray in it, 
