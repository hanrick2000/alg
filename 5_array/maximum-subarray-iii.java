...
有两种情况:
第一种是第i个元素自己组成一个子数组, 则要在前i－1个元素中找k－1个子数组:
  globalMax[i-1][k-1] + nums[i-1]
第二种情况是第i个元素属于前一个元素的子数组, 因此要在i－1个元素中找k个子数组
(并且必须包含第i－1个元素,这样第i个元素才能合并到最后一个子数组中):
...
有两种情况:
第一种是不包含第i个元素,所以要在前i－1个元素中找k个子数组,
第二种情况为包含第i个元素,在i个元素中找k个子数组且必须包含第i个元素,
取两种情况里面大的那个.
...
Tags: Dynamic Programming, Subarray, Array
*/
