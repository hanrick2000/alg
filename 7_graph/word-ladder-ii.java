public class Solution {
    public List<List<String>> findLadders(String start, 
        ...
        Map<String, List<String>> map = new HashMap<String, List<String>>(); //map内记录每个单词, 可以由哪些单词经过一步变换得到
        Map<String, Integer> distance = new HashMap<String, Integer>(); //distance记录每个单词距离start的最短距离
        dict.add(start);
        dict.add(end);
    ...
    void bfs(Map<String, List<String>> map, 
             ...
             String start, 
             String end, 
             Set<String> dict) { //填充map和distance
        ...
        while (!queue.isEmpty()) {
            ...
            for (String nextWord: getNextWords(word, dict)) {
                ...
                if (!distance.containsKey(nextWord)) { //先放进distance的一定是这个单词到达start最短的距离
...
思路:
求最短距离的所有解，利用BFS + DFS。
1) BFS过程与Word Ladder类似，但是在BFS过程中利
用一个HashMap(distance)记录每个单词距start的最短距离，
另一个HashMap(map)记录每个单词是由哪些单词变换一步得到的。

2) DFS过程类似Permutation，从end出发，逐个寻找其上一节点直到start。
其中可能的上一节点可以由1）中的map得到，但是只有距离比当前节点距离小1的上一节点才是合法的
（保证其一定在end和start之间的最短路径上，即找到的下一个点一定比当前点离start更近，这样防止搜到的下一个点向外走），距离可以由1）中的distance得到。
...
因此没有元素会被加到map和distance中。在lintcode里可以过，但是在leetcode里过不了。
