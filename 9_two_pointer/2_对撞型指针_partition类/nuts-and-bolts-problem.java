public class Solution {
    ...
    private void qsort(String[] nuts, String[] bolts, NBComparator compare, int l, int r) {
        ...
        int position = partition(nuts, bolts[l], compare, l, r);//返回bolts[l]对应的nut在nut[]中的index, 利用bolts[l]把nut划分成了两份, index左边是<=bolts[l](对应的nut)的, index右边是>=bolts[l]的(nut)
        partition(bolts, nuts[position], compare, l, r);
        qsort(nuts, bolts, compare, l, position - 1);
        qsort(nuts, bolts, compare, position + 1, r);
    }
...
public class Solution {
    ...
    private int partition(String[] str, String pivot, NBComparator compare, int l, int r) {
        ...
        int left = l; 
        int right = r;
        String now = str[left]; //因为预先保存了str[lelft], 则必须先从右边开始找第一个大于pivot的, 这里是跟qsort的partition不一样的地方, now只存储了后面被覆盖的元素, 而用于比较的pivot这里不是now, 而是传进来用于间接比较的pivot, 这个now实际上是pivot对应的另个数组的元素
