public class Solution {
    public boolean wordBreak(String s, Set<String> dict) {
        ...
        //state
        boolean[] f = new boolean[s.length() + 1]; //f[i]表示前i个位置(包括i), 能否被dict完美切分, 因为要想要保存一个f[0]空串的状态, 所以多用一个空间
        //init
        f[0] = true;
        //fuction
        for (int i = 1; i <= s.length(); i++) {//遍历输入s, O(N)
            for (int lastWordLength = 1;
                ...
                String word = s.substring(i - lastWordLength, i); //再看后半部分嫩不能被完美切分, substring返回[i-lastWordLength, i]闭区间内的字符串, O(L)
