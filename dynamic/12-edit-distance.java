...
public class Solution {
    public int minDistance(String word1, String word2) {
        ...
        int m = word2.length();
        //state
        int[][] dp = new int[n+1][m+1]; //dp[i][j]表示A的前i个字符最少要用几次编辑变成B的前j个字符
        //init
        for(int i=0; i < m+1; i++) dp[0][i] = i; //把一个空串变成i长度的, 需要i次编辑
        ...
        for(int i = 1; i <= n; i++){
            for(int j=1; j <= m; j++){
                if(word1.charAt(i-1) == word2.charAt(j-1)){
                    ...
                    dp[i][j] = 1 + Math.min(dp[i-1][j-1], Math.min(dp[i-1][j], dp[i][j-1])); //不等, 从三者里面选出最小的, 然后加1
                                           //replace      del                  insert
                }
            }
        }
